<template>
  <div>
    <div style="margin-bottom:10px">
      ...
      <el-button type="success" slot="reference" @click="dialogVisible=true">新增教室授课</el-button>
      ...
      <el-upload
          action="http://localhost:9090/teach/importStudentFile"
          :show-file-list="false"
          accept=".xlsx,.xls"
          style="display: inline-block"
          :on-success="handleExcelImportSuccess">
        <el-button type="success">导入教师授课<i class="el-icon-bottom"></i></el-button>
      </el-upload>
    </div>
    ...
    <el-table :data="tableData"
      ...
      <el-table-column prop="tid" label="教工号" width="150px" sortable/>
    ...
    <div>
      <el-dialog
        ...
        <el-form :model="form"
          ...
          <el-form-item label="教工号" label-width="120px">
            <el-input v-model="form.tid" autocomplete="off" style="width: 250px" clearable></el-input>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    // 刷新与重置
    load() {
      this.request.get("http://localhost:9090/teach/page?", {
        params: {
    ...
    // CRUD
    handleDelete(row) {
      this.request.post("http://localhost:9090/teach/delete",row).then(res => {
        if (res.code== '200') {
    ...
    handleDelBatch() {
      ...
      this.request.post("http://localhost:9090/teach/delbatch/", users).then(res => {
        console.log(1)
        if (res != 0) {
    ...
    async save(){
      await this.request.post("http://localhost:9090/teach/handleAdd", this.form).then(res => {
        console.log(res)
        if (res.code === '200') {
