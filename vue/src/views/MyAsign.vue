      ...
      <el-table-column label="操作" width="220 px">
        <template slot-scope="scope">
          <el-popconfirm title="确定这条信息吗？" class="ml5 mr5"
            ...
            <el-button type="warning" slot="reference">撤回 <i class="el-icon-remove-outline"></i></el-button>
          </el-popconfirm>
...
export default {
  ...
  methods: {
    ...
    // CRUD
    handleDelete(asgid) {
      this.request.delete("http://localhost:9090/asign/revoke/" + asgid).then(res => {
        if (res.code==='200') {
